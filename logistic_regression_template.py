...
if __name__ == '__main__':
    ...
    # TODO generate plots
    x_axis = np.arange(hyperparameters['num_iterations'])
    plt.plot(x_axis,logging[:,1], "r", label = "cross_entropy_train")
    plt.plot(x_axis, logging[:,3], "b", label = "cross_entropy_valid")
    plt.xlabel("Number of Iteration")
    plt.ylabel("Cross Entropy")
    plt.title("Validation set on mnist_train_small")
    plt.legend()
    plt.show()